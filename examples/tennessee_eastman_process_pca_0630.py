# -*- coding: utf-8 -*-
"""Tennessee Eastman process_PCA_0630

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CKEnKH5qF7olpATbC3gTfU9-wyXEJjPH
"""
...
Data_train_nolabel = scaler.fit_transform(data_train_np[:,0:52])
label_train = data_train_np[:,52]
...
# Check variance explained by having different principal components
pca_52 = PCA(n_components=52, random_state=42)
pca_52.fit(Data_train_nolabel)
print(np.cumsum(pca_52.explained_variance_ratio_*100))
plt.plot(np.cumsum(pca_52.explained_variance_ratio_*100))
plt.xlabel('Number of components')
plt.ylabel('Explained variance')
plt.grid(True)
plt.show()

# Choose a principal components number which is considered sufficient
...
lower_bound = 0
upper_bound = max(reconstruction_error_train)
...
while upper_bound - lower_bound > tolerance:
    ...
    anomalies_tuning = np.where(np.array(reconstruction_error_train) > current_threshold)[0]
...
# Compute the confusion matrix for performance_total
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Total_Confusion Matrix_PCA')
# Show the plot
plt.show()